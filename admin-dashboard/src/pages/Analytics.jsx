...
function KpiCard({ icon: Icon, label, value, sub, color }) {
  ...
  return (
    <Card className="p-5 flex items-center gap-4">
      <div className={`w-11 h-11 ${c.bg} ring-1 ${c.ring} rounded-2xl flex items-center justify-center flex-shrink-0`}>
    ...
    <div className="p-5 lg:p-7 space-y-7">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <Card className="lg:col-span-2 p-5">
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">Monthly UGX earnings</p>
            </div>
          ...
          <div className="mt-4 border-t border-blue-500/10 pt-3 flex items-center justify-between">
            <span className="text-xs text-slate-500">Total projects per month shown in bars</span>
            <span className="text-xs font-bold text-blue-400">Peak: UGX {(maxRev/1000000).toFixed(1)}M</span>
          </div>
        </Card>
        ...
        <Card className="p-5">
          <div className="mb-5">
            ...
            <p className="text-xs text-slate-500 mt-0.5">2026 breakdown</p>
          </div>
          <DonutChart data={serviceBreakdown} />
          <div className="mt-5 pt-4 border-t border-blue-500/10">
            <div className="text-xs text-slate-500">Web Development leads at <span className="text-blue-400 font-bold">42%</span> of all revenue.</div>
          </div>
        </Card>
      ...
      <Card className="overflow-hidden">
        <div className="p-4 border-b border-blue-500/10">
          <h3 className="font-bold text-white text-sm">Monthly Breakdown</h3>
        </div>
            ...
            <thead>
              <tr className="border-b border-blue-500/10">
                {['Month','Revenue (UGX)','Projects','Avg. per Project','Revenue Bar'].map(h => (
                  <th key={h} className="text-left text-[11px] font-bold text-slate-500 uppercase tracking-widest px-5 py-3">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-blue-500/5">
              {monthlyProjects.map((row) => (
                <tr key={row.month} className="hover:bg-slate-700/20 transition-colors">
                  <td className="px-5 py-3 font-semibold text-white text-sm">{row.month}</td>
                  <td className="px-5 py-3 text-sm text-blue-400 font-bold font-mono">
                    {(row.revenue / 1000000).toFixed(2)}M
                  </td>
                  <td className="px-5 py-3 text-sm text-slate-300">{row.projects}</td>
